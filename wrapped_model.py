...
class WrappedReadingVecModel(torch.nn.Module):
    ...
    def contrast_generate(self, prompt=None,
                                input_ids=None,
                                attention_mask=None,
                                coeffic=0.5,
                                tokenizer=None,
        ...
        pos_input = positive_prompt + ' ' + prompt
        neg_input = negative_prompt + ' ' + prompt
        control_input = prompt

        enc = tokenizer(control_input, return_tensors="pt").to(self.model.device)
        query_length = enc['input_ids'].size(1)

        pos_enc = tokenizer(pos_input, return_tensors="pt").to(self.model.device)
        neg_enc = tokenizer(neg_input, return_tensors="pt").to(self.model.device)
        ...
        direction_hidden = {}
        for l in layer_ids:
            direction_hidden[l] = coeffic * (pos_outputs[l][:, -query_length:] - neg_outputs[l][:, -query_length:])
        self.control_on_layers(
                layer_ids=layer_ids,
                grads=direction_hidden,
                query_length=query_length,
                token_pos='start'
                #gradient_manipulation=gradient_manipulation,
                #epsilon=epsilon
                )
        contrast_output = self.generate(**enc, use_cache=use_cache, do_sample=do_sample, **kwargs)
    ...
    def get_suffix_score(self, prompt, suffix, **kwargs):
        ...
        for prob in probs:
            target_idx = torch.where(torch.tensor([yes_token, no_token]) == targets[0])[0]
            neg_target_idx = torch.where(torch.tensor([yes_token, no_token]) != targets[0])[0]
